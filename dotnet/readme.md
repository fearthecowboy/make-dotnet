...
# .NET Core {version} {sdkorruntime}

This package installs the .NET Core {version} {sdkorruntime} for your platform and provides the `dotnet` executable.

No elevation/root required.

Partners with the [dotnet{flavor}{version}](https://www.npmjs.com/package/dotnet{flavor}{version}) package.
...
npm i -g dotnet-{version}      // for running dotnet apps
npm i -g dotnet-sdk-{version}  // for developing dotnet apps
```
...
The following `package.json` uses both the `dotnet-sdk-{version}` and `dotnet-{version}` in order to simplify both developing and shipping the package.
...
We created several *platform-specific* helper packages that actually provide the promised `dotnet` installation (e.g. `dotnet-sdk-{version}-win`).
...
By default, the installation location is `~/.net/{version}` since it is accessible without elevation.
